...
export default function Experience() {
  ...
  // Simplified certificates with direct image URLs
                  ...
                  >
                    <div className="glassmorphism hover:shadow-accent-indigo/20 aspect-[4/3] overflow-hidden rounded-xl transition-all duration-300 hover:shadow-lg">
                      <img
                        ...
                        onError={(e) => {
                          console.error(`❌ ${cert.title} failed to load`);
                          // Replace with text fallback
                          const target = e.currentTarget;
                          const parent = target.parentElement;
                          if (parent) {
                            parent.innerHTML = `
                              <div class="h-full w-full flex items-center justify-center bg-slate-700 text-slate-400 text-center p-2">
                                <div>
              ...
              >
                ...
                >
                  ...
                  <img
                    ...
                    onError={(e) => {
                      console.error(
                        `❌ Modal image failed: ${certifications[selectedCert].title}`
                      );
                      const target = e.currentTarget;
                      const parent = target.parentElement;
                      if (parent) {
                        parent.innerHTML = `
                          <div class="h-full w-full flex items-center justify-center bg-slate-700 text-white text-center">
                            <div>
                              <div class="text-4xl mb-4">🏆</div>
                              <h3 class="text-xl font-bold">${certifications[selectedCert].title}</h3>
                            </div>
                          </div>
                          <button onclick="document.querySelector('.fixed').style.display='none'" class="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white">×</button>
                        `;
                      }
                    }}
                  />
